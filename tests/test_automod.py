...
def test_modify_sql_with_auto_modulo():
    assert clean(modify_sql_with_auto_modulo("""
        ...
        """, 8)) == clean("""
            UPDATE foo SET bar = baz WHERE (id % 8 = 0); --&
            UPDATE foo SET bar = baz WHERE (id % 8 = 1); --&
            UPDATE foo SET bar = baz WHERE (id % 8 = 2); --&
            UPDATE foo SET bar = baz WHERE (id % 8 = 3); --&
            UPDATE foo SET bar = baz WHERE (id % 8 = 4); --&
            UPDATE foo SET bar = baz WHERE (id % 8 = 5); --&
            UPDATE foo SET bar = baz WHERE (id % 8 = 6); --&
            UPDATE foo SET bar = baz WHERE (id % 8 = 7); --&
        """)
    ...
    assert clean(modify_sql_with_auto_modulo("""
        ...
        """, 8)).startswith(clean("""
            ...
            SET merged_osm_id = linked_node_osm_id,
                ...
                wikidata = COALESCE(NULLIF(polygon.wikidata, ''), linked_node_wikidata)
            FROM polygons_with_linked_by_relation_node
            WHERE polygon_id = polygon.id AND (polygon.id % 8 = 0); --&
        """))
